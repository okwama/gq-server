...
// Helper function to get client discount percentage
const getClientDiscountPercentage = async (clientId) => {
  ...
  return client?.discountPercentage || 0;
};
  ...
  upload(req, res, async function(err) {
    ...
    try {
      ...
      for (const item of orderItemsToUse) {
        ...
        // Get product with store quantities and store details first
        ...
        // Handle price option validation - check if priceOptionId is provided
        let priceOption = null;
        if (item.priceOptionId) {
          // First get the price option to validate it exists
          priceOption = await prisma.priceOption.findUnique({
          ...
          console.log('[Order Debug] No price option provided, will use product unit_cost as fallback');
        }
      ...
      try {
        ...
        const clientDiscountPercentage = await getClientDiscountPercentage(clientId);
        console.log('[Order Debug] Client discount percentage:', clientDiscountPercentage);
        ...
        for (const item of orderItemsToUse) {
          ...
          if (item.priceOptionId) {
            ...
            if (priceOption) {
              originalPrice = priceOption.value || 0;
              itemPrice = calculateDiscountedPrice(originalPrice, clientDiscountPercentage);
              priceOptionId = priceOption.id;
              
              console.log('[Order Debug] Price calculation with price option:', {
                productId: product.id,
                productName: product.name,
                priceOptionId: priceOption.id,
                originalPrice,
              ...
              console.log(`[Order Debug] Price option ${item.priceOptionId} not found for product ${product.id}, using unit_cost as fallback`);
              // Fallback to unit_cost if price option not found
              originalPrice = Number(product.unit_cost) || 0;
              itemPrice = calculateDiscountedPrice(originalPrice, clientDiscountPercentage);
              
              console.log('[Order Debug] Price calculation with unit_cost fallback:', {
                productId: product.id,
                productName: product.name,
            ...
            // No price option provided, use unit_cost as fallback
            originalPrice = Number(product.unit_cost) || 0;
            itemPrice = calculateDiscountedPrice(originalPrice, clientDiscountPercentage);
            
            console.log('[Order Debug] Price calculation with unit_cost (no price option):', {
              productId: product.id,
              productName: product.name,
              unit_cost: product.unit_cost,
              originalPrice,
...
const updateOrder = async (req, res) => {
  ...
  try {
    ...
    // Get client discount percentage
    const clientDiscountPercentage = await getClientDiscountPercentage(existingOrder.clientId);
    console.log('[Update Order Debug] Client discount percentage:', clientDiscountPercentage);
      ...
      for (const item of orderItems) {
        ...
        if (item.priceOptionId) {
          ...
          if (priceOption) {
            ...
            console.log('[Update Order Debug] Price calculation with price option:', {
              productId: product.id,
              productName: product.name,
              priceOptionId: priceOption.id,
              originalPrice,
            ...
            console.log(`[Update Order Debug] Price option ${item.priceOptionId} not found for product ${product.id}, using unit_cost as fallback`);
            // Fallback to unit_cost if price option not found
            originalPrice = Number(product.unit_cost) || 0;
            itemPrice = calculateDiscountedPrice(originalPrice, clientDiscountPercentage);
            
            console.log('[Update Order Debug] Price calculation with unit_cost fallback:', {
              productId: product.id,
              productName: product.name,
              unit_cost: product.unit_cost,
              originalPrice,
          ...
          console.log('[Update Order Debug] Price calculation with unit_cost (no price option):', {
            productId: product.id,
            productName: product.name,
            unit_cost: product.unit_cost,
            originalPrice,
...
// User endpoint to request void for an order
const requestOrderVoid = async (req, res) => {
  try {
    const orderId = parseInt(req.params.id);
    const userId = req.user.id;
    const reason = req.body.reason || 'Customer requested void';

    // Find order and verify ownership
    const order = await prisma.myOrder.findFirst({
      where: {
        id: orderId,
        userId: userId
      }
    ...
    // Check if order is in a state that can be voided
    if (order.status !== 0) {
      let message = 'Order cannot be voided';
      if (order.status === 4) message = 'Void request already pending';
      if (order.status === 5) message = 'Order already voided';
    ...
    // Update order to void requested status
    await prisma.myOrder.update({
      where: { id: orderId },
      data: {
        status: 4, // Void Requested
...
// User endpoint to check void status
const checkVoidStatus = async (req, res) => {
  try {
    const orderId = parseInt(req.params.id);
    const userId = req.user.id;
    ...
    switch (order.status) {
      case 0:
        statusInfo.message = 'Active';
        statusInfo.canVoid = true;
        break;
      case 4:
        statusInfo.message = 'Void request pending approval';
        break;
      case 5:
        statusInfo.message = 'Order voided';
        break;
      case 6:
        statusInfo.message = 'Void request rejected';
        break;
      default:
        statusInfo.message = 'Order cannot be voided';
    }
    ...
    res.json({
      ...
      canRequestVoid: statusInfo.canVoid,
